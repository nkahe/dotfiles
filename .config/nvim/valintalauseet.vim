...
inoremap  :edit $DOTFILES/vim/vimrc
nnoremap J mzJ`z
nnoremap Y y$
...
map! [3~ <Del>
snoremap  :edit $DOTFILES/vim/vimrc
nnoremap  :edit $DOTFILES/vim/vimrc
nnoremap î :NERDTreeFocus
let &cpo=s:cpo_save
unlet s:cpo_save
set autoindent
set autoread
...
setlocal statusline=%{lightline#link()}%#LightlineLeft_active_0#%(\ %{lightline#mode()}\ %)%{(&paste)?\"\":\"\"}%(\ %{&paste?\"PASTE\":\"\"}\ %)%#LightlineLeft_active_0_1#%#LightlineLeft_active_1#%(\ %R\ %)%{((&filetype!=\"help\"&&\ &readonly))&&(1||((&filetype!=\"help\"&&(&modified||!&modifiable))))?\"\":\"\"}%(\ %f\ %)%{((&filetype!=\"help\"&&(&modified||!&modifiable)))?\"\":\"\"}%(\ %M\ %)%#LightlineLeft_active_1_2#%#LightlineMiddle_active#%=%#LightlineRight_active_2_3#%#LightlineRight_active_2#%(\ %{&ff}\ %)%{1||1?\"\":\"\"}%(\ %{&fenc!=#\"\"?&fenc:&enc}\ %)%{1?\"\":\"\"}%(\ %{&ft!=#\"\"?&ft:\"no\ ft\"}\ %)%#LightlineRight_active_1_2#%#LightlineRight_active_1#%(\ %3p%%\ %)%#LightlineRight_active_0_1#%#LightlineRight_active_0#%(\ %3l:%-2v\ %)
